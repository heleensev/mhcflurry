...
DOWNLOAD_NAME=models_class1_pan_allele_cluster
...
cp /data/cmbi/dlepikhov/3DVac_experiments/experiments_data/BA_pMHCI_human_quantitative_only_eq_pseudoseq_clustered_train_validation.csv .
TRAINING_DATA="$(pwd)/BA_pMHCI_human_quantitative_only_eq_pseudoseq_clustered_train_validation.csv"
...
do
    ...
    mhcflurry-class1-train-pan-allele-models \
        ...
        --out-models-dir "/data/cmbi/dlepikhov/3DVac_experiments/allele_clustered_exp/models.unselected.${kind}" \
...
for kind in combined
do
    MODELS_DIR="/data/cmbi/dlepikhov/3DVac_experiments/allele_clustered_exp/models.unselected.${kind}"
...
mkdir .ignored
mv "/data/cmbi/dlepikhov/3DVac_experiments/allele_clustered_exp/models.unselected.${kind}" .ignored/
RESULT="$SCRATCH_DIR/${DOWNLOAD_NAME}.selected.$(date +%Y%m%d).tar.bz2"
tar -cjf "$RESULT" *
mv .ignored/* . && rmdir .ignored
mv "$RESULT" "/data/cmbi/dlepikhov/3DVac_experiments/allele_clustered_exp/"
